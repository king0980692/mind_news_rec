...
def build_meta():
    ...
    if args.verbose:
        print("\t ... Tokenize news title & abstract by {}".format(args.tokenizer))
        ...
        with open(news_file, 'r', encoding='utf-8') as news_f:
            for line in news_f:
                ...
                if news_ID not in news_ID_set:
                    ...
                    # tokenize
                    words = pat.findall((title + ' ' + abstract).lower()) if tokenizer == 'MIND' else tknzr.tokenize((title + ' ' + abstract).lower())
    ...
    if args.verbose:
        print("\t ... Ensemble token into token and create tfidf table")
    ...
    if args.verbose:
        print("\t ... Fit and transform sentence") 
    tfidf_matrix = vectorizer.fit_transform(sentence_corpus)

    if args.verbose:
        print("\t ... Create history dict")
    user_history_dict = {}
...
def generate_user_tfidf(news_tfidf, user_history_dict):
    if args.verbose:
        print('\t ... trans every history-news token to tfidf encoding')
    user_tfidf = {}
...
if __name__ == '__main__':
    ...
    if args.verbose:
        print("[Generate news tfidf]")
    news_tfidf = generate_news_tfidf(news_dict, tfidf_matrix)

    if args.verbose:
        print("\t ... dump news tfidf table to file")
    with open('./exp/news_tfidf-%s.pkl' % dataset, 'wb') as news_tfidf_f:
    ...
    if args.verbose:
        print("\t ... dump user tfidf table to file")
    with open('./exp/user_tfidf-%s.pkl' % dataset, 'wb') as user_tfidf_f:
