...
random.seed(0)
np.random.seed(0)
MIND_small_dataset_root = './data/MIND-small'
MIND_large_dataset_root = './data/MIND-large'
MIND_200k_dataset_root = './data/MIND-200k'
...
def download_extract_MIND_small():
    ...
    if not os.path.exists(MIND_small_dataset_root + '/download/train'):
        if not os.path.exists(MIND_small_dataset_root + '/download/MINDsmall_train.zip'):
            ...
            print(MIND_small_dataset_root + '/download/MINDsmall_train.zip'+ 'exist !')
    ...
    if not os.path.exists(MIND_small_dataset_root + '/download/dev'):
        if not os.path.exists(MIND_small_dataset_root + '/download/MINDsmall_dev.zip'):
            ...
            print(MIND_small_dataset_root + '/download/MINDsmall_dev.zip'+ 'exist !')
...
def download_extract_MIND_large(mode):
    ...
    data_path = MIND_large_dataset_root if mode=='large' else MIND_200k_dataset_root
    if not os.path.exists(data_path):
        os.mkdir(data_path)
    if not os.path.exists(data_path+'/train'):
        os.mkdir(data_path+'/train')
    if not os.path.exists(data_path+'/dev'):
        os.mkdir(data_path+'/dev')
    ...
    if mode == 'large':
        for data in ['train', 'dev', 'test']:
            shutil.copyfile(MIND_large_dataset_root + '/download/%s/news.tsv' % data, data_path+'/%s/news.tsv' % data)
            shutil.copyfile(MIND_large_dataset_root + '/download/%s/behaviors.tsv' % data, data_path+'/%s/behaviors.tsv' % data)
...
def sampling_MIND_dataset():
    sample_num = 200000
    large_data_path = MIND_large_dataset_root
    

    # 1. randomly sample by users
    user_set = set()
    with open(large_data_path+'/train/behaviors.tsv', 'r', encoding='utf-8') as f:
        for line in f:
            impression_ID, user_ID, time, history, impressions = line.strip().split('\t')
            user_set.add(user_ID)
    with open(large_data_path+'/dev/behaviors.tsv', 'r', encoding='utf-8') as f:
        for line in f:
            impression_ID, user_ID, time, history, impressions = line.strip().split('\t')
            user_set.add(user_ID)
    ...
    with open(MIND_200k_dataset_root+'/sample_users.json', 'w', encoding='utf-8') as f:
        json.dump(sample_user_list, f)
    sampled_user_set = set(sample_user_list)
    # 2. write sampled behavior file
    with open(large_data_path+'/train/behaviors.tsv', 'r', encoding='utf-8') as f:
        with open(MIND_200k_dataset_root+'/train/behaviors.tsv', 'w', encoding='utf-8') as train_f:
            for line in f:
                impression_ID, user_ID, time, history, impressions = line.strip().split('\t')
    ...
    cnt = 0
    with open(large_data_path+'/dev/behaviors.tsv', 'r', encoding='utf-8') as f:
        with open(MIND_200k_dataset_root+'/dev/behaviors.tsv', 'w', encoding='utf-8') as dev_f:
            with open(MIND_200k_dataset_root+'/test/behaviors.tsv', 'w', encoding='utf-8') as test_f:
                for line in f:
                    impression_ID, user_ID, time, history, impressions = line.strip().split('\t')
                    if user_ID in sampled_user_set:
                        if cnt % 2 == 0:
    ...
    for mode in ['train', 'dev', 'test']:
        with open(MIND_200k_dataset_root+'/%s/behaviors.tsv' % (mode), 'r', encoding='utf-8') as f:
            news_set = set()
            for line in f:
                impression_ID, user_ID, time, history, impressions = line.strip().split('\t')
                if len(history) > 0:
                ...
                if len(impressions) > 0:
                    ...
                    for n in news:
                        news_set.add(n[:-2])
            with open(large_data_path+'/%s/news.tsv' % ('dev' if mode == 'test' else mode), 'r', encoding='utf-8') as _f:
                with open(MIND_200k_dataset_root+'/%s/news.tsv' % mode, 'w', encoding='utf-8') as __f:
                    for line in _f:
