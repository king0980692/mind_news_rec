...
def build_meta():
    ...
    with open('exp/news_ID-%s.pkl' % dataset, 'wb') as news_ID_f:
        pickle.dump(news_ID_dict, news_ID_f)
    with open('exp/user_ID-%s.pkl' % dataset, 'wb') as user_ID_f:
        pickle.dump(user_ID_dict, user_ID_f)
    with open('exp/offset-%s.txt' % dataset, 'w', encoding='utf-8') as f:
        f.write(str(len(news_ID_dict)) + '\n')
        f.write(str(len(user_ID_dict)) + '\n')
        f.write(str(len(vectorizer.get_feature_names_out())) + '\n')
