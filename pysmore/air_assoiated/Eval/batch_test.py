...
data_generator = Data(path=args.data_path + args.dataset , batch_size=256)
...
def test_rel_ui( users_to_test, U, I, R, purc_rel_to_test ):
    result = {'precision': np.zeros(len(Ks)), 'recall': np.zeros(len(Ks)), 'ndcg': np.zeros(len(Ks))}
...
def test_rel_uir( users_to_test, U, I, RU, RI, purc_rel_u_to_test, purc_rel_i_to_test ):
    result = {'precision': np.zeros(len(Ks)), 'recall': np.zeros(len(Ks)), 'ndcg': np.zeros(len(Ks))}
    ...
    for u_batch_id in range(n_user_batchs):
        ...
        item_batch = range(ITEM_NUM)
        #print('user & real_u_batch:', [i*3 for i in user_batch]==rel_u_batch)
        #print('item & real_i_batch:', [i*3 for i in item_batch]==rel_i_batch)
