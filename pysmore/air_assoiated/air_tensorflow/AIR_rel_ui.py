...
class BPRMF(object):
    def __init__(self, data_config, pretrain_data):
        ...
        self.u_r_idx = tf.placeholder(tf.int32, shape=(None,)) # 
        self.pos_u_r_idx = tf.placeholder(tf.int32, shape=(None,))
        self.neg_u_r_idx = tf.placeholder(tf.int32, shape=(None,))

        self.i_r_idx = tf.placeholder(tf.int32, shape=(None,)) # 
        self.pos_i_r_idx = tf.placeholder(tf.int32, shape=(None,))
        self.neg_i_r_idx = tf.placeholder(tf.int32, shape=(None,))
        ...
        self.u_rel_embeddings = self.weights['user_rel_embedding']
        self.i_rel_embeddings = self.weights['item_rel_embedding']
        ...
        self.ur_embeddings = tf.nn.embedding_lookup(self.u_rel_embeddings, self.u_r_idx)
        self.pos_ur_embeddings = tf.nn.embedding_lookup(self.u_rel_embeddings, self.pos_u_r_idx)
        self.neg_ur_embeddings = tf.nn.embedding_lookup(self.u_rel_embeddings, self.neg_u_r_idx)

        self.ir_embeddings = tf.nn.embedding_lookup(self.i_rel_embeddings, self.i_r_idx)
        self.pos_ir_embeddings = tf.nn.embedding_lookup(self.i_rel_embeddings, self.pos_i_r_idx)
        self.neg_ir_embeddings = tf.nn.embedding_lookup(self.i_rel_embeddings, self.neg_i_r_idx)

        # All ratings for all users.
        # (u+r)* (i+r)
        self.batch_ratings = tf.matmul(tf.add(self.u_g_embeddings , self.ur_embeddings), tf.add(self.pos_i_g_embeddings, self.ir_embeddings,), transpose_a=False, transpose_b=True)
        ...
        self.mf_loss, self.emb_loss = self.create_bpr_loss(self.u_g_embeddings,
                                                            ...
                                                            self.ur_embeddings,
                                                            self.pos_ur_embeddings,
                                                            self.neg_ur_embeddings,
                                                            self.ir_embeddings,
                                                            self.pos_ir_embeddings,
                                                            self.neg_ir_embeddings)       
    ...
    def _init_weights(self):
        ...
        all_weights['user_rel_embedding'] = tf.Variable(initializer([self.n_users * 3, self.emb_dim]), name='user_rel_embedding')
        all_weights['item_rel_embedding'] = tf.Variable(initializer([self.n_items * 3, self.emb_dim]), name='user_item_embedding')

        return all_weights

    def create_bpr_loss(self, users, pos_users, neg_users, items, pos_items, neg_items, u_rel_idx, u_pos_rel_idx, u_neg_rel_idx, i_rel_idx, i_pos_rel_idx, i_neg_rel_idx ):
        
        # g = u+i+r
        u_i_term = tf.add( tf.add(users, u_rel_idx), tf.add(items, i_rel_idx) ) # g
        u_i_pos_term = tf.add( tf.add(pos_users, u_pos_rel_idx), tf.add(pos_items, i_pos_rel_idx) ) # g+
        u_i_neg_term = tf.add( tf.add(neg_users, u_neg_rel_idx), tf.add(neg_items, i_neg_rel_idx ) ) # g-
        ...
        regularizer = tf.nn.l2_loss(users) + tf.nn.l2_loss(pos_users) + tf.nn.l2_loss(neg_users) + \
                      ...
                      tf.nn.l2_loss(u_rel_idx) +tf.nn.l2_loss(u_pos_rel_idx) + tf.nn.l2_loss(u_neg_rel_idx) + \
                      tf.nn.l2_loss(i_rel_idx) +tf.nn.l2_loss(i_pos_rel_idx) + tf.nn.l2_loss(i_neg_rel_idx)                     
...
if __name__ == '__main__':
    ...
    for epoch in range(1, args.epoch + 1):
        ...
        for idx in range(n_batch):
            ...
            users, items, pos_users, pos_items, neg_users, neg_items, u_rel_idx, pos_u_rel_idx, neg_u_rel_idx, i_rel_idx, pos_i_rel_idx, neg_i_rel_idx = data_generator.sample_negrel_ui()
            _, batch_loss, batch_mf_loss, batch_emb_loss = sess.run([model.opt, model.loss, model.mf_loss, model.emb_loss],
                                                        feed_dict={model.users: users, model.pos_users: pos_users, model.neg_users: neg_users, 
                                                                    ...
                                                                    model.u_r_idx: u_rel_idx,
                                                                    model.pos_u_r_idx: pos_u_rel_idx,
                                                                    model.neg_u_r_idx: neg_u_rel_idx,
                                                                    model.i_r_idx: i_rel_idx,
                                                                    model.pos_i_r_idx: pos_i_rel_idx,
                                                                    model.neg_i_r_idx: neg_i_rel_idx })
        ...
        purc_rel_i_to_test = [ 3*us + 0   for us in range(data_generator.n_items)] # +0 : purchase, +1: pv, +2: cart
        ret = test_rel_ui(sess, model, users_to_test , purc_rel_to_test, purc_rel_i_to_test, drop_flag=False)
