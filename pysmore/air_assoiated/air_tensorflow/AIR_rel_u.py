...
class BPRMF(object):
    def __init__(self, data_config, pretrain_data):
        ...
        self.rel_embeddings = self.weights['user_rel_embedding']
        ...
        self.r_embeddings = tf.nn.embedding_lookup(self.rel_embeddings, self.u_r_idx)
        self.pos_r_embeddings = tf.nn.embedding_lookup(self.rel_embeddings, self.pos_u_r_idx)
        self.neg_r_embeddings = tf.nn.embedding_lookup(self.rel_embeddings, self.neg_u_r_idx)

        # All ratings for all users.
        # (u+r)*i
        self.batch_ratings = tf.matmul(tf.add(self.u_g_embeddings , self.r_embeddings), self.pos_i_g_embeddings, transpose_a=False, transpose_b=True)
        ...
        self.mf_loss, self.emb_loss = self.create_bpr_loss(self.u_g_embeddings,
                                                            ...
                                                            self.r_embeddings,
                                                            self.pos_r_embeddings,
                                                            self.neg_r_embeddings)
    ...
    def create_bpr_loss(self, users, pos_users, neg_users, items, pos_items, neg_items, u_rel_idx, u_pos_rel_idx, u_neg_rel_idx ):
        
        # g = u+i+r
        u_i_term = tf.add( tf.add(users, items), u_rel_idx ) # g
        u_i_pos_term = tf.add( tf.add(pos_users, pos_items), u_pos_rel_idx ) # g+
        u_i_neg_term = tf.add( tf.add(neg_users, neg_items), u_neg_rel_idx ) # g-
...
if __name__ == '__main__':
    ...
    for epoch in range(1, args.epoch + 1):
        ...
        for idx in range(n_batch):
            ...
            users, items, pos_users, pos_items, neg_users, neg_items, u_rel_idx, pos_u_rel_idx, neg_u_rel_idx = data_generator.sample_negrel()
        ...
        ret = test_rel(sess, model, users_to_test , purc_rel_to_test, drop_flag=False)
