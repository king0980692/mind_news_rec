...
from mind_dataloader import MIND_Dataset
...
def test_with_no_ans(model, data_loader, device, truth_txt):

    targets, predicts = list(), list()
    with open(truth_txt) as f:
        for line in f.readlines():
            lid, r_list = line.split()
            t = json.loads(r_list)
            targets.extend(t)
    ...
    with open("exp/tfm.pred", 'w') as f:
        f.writelines("\n".join(list(map(str, predicts))))
...
def main(dataset_path,
    ...
    u_enc = preprocessing.OrdinalEncoder(handle_unknown='use_encoded_value',
                                 unknown_value=-1)
    n_enc = preprocessing.OrdinalEncoder(handle_unknown='use_encoded_value',
                                 unknown_value=-1)
    cat_enc = preprocessing.OrdinalEncoder(handle_unknown='use_encoded_value',
                                 unknown_value=-1)
    scat_enc = preprocessing.OrdinalEncoder(handle_unknown='use_encoded_value',
                                 unknown_value=-1)

    print("Prepare dataset ...")
    dataset = MIND_Dataset(dataset_path, "exp/news_map.pkl", u_enc, n_enc, cat_enc, scat_enc) 




    # test_dataset = MIND_Dataset("../MIND-sample/test/behaviors.tsv")
    ...
    # Load batch data
    train_data_loader = DataLoader(train_dataset, batch_size=batch_size, num_workers=16)
    valid_data_loader = DataLoader(valid_dataset, batch_size=batch_size, num_workers=16)
    ...
    model.load_state_dict(torch.load(f"chkpt/{args.model_name}.pt"))

    print("Prepare test dataset ...")
    test_dataset = MIND_Dataset("./data/dev/behaviors.tsv", "exp/news_map.pkl",u_enc, n_enc, cat_enc, scat_enc, is_train=False)
    test_data_loader = DataLoader(test_dataset, batch_size=batch_size, num_workers=8)

    print("Testing ...")
    auc = test_with_no_ans(model, test_data_loader, device, "./data/dev/truth.txt")
