...
def train(rank, args):
    ...
    for ep in range(args.start_epoch, args.epochs):
        ...
        for cnt, (log_ids, log_mask, input_ids, targets) in enumerate(dataloader):
            ...
            #                     (history, history_mask, candidate, label):
...
def test(rank, args):
    ...
    model = module.Model(args, dummy_embedding_matrix)
    ...
    def collate_fn(tuple_list):
        ...
        return (log_vecs, log_mask, news_vecs)

    dataset = DatasetVal(data_file_path, news_index, news_scoring, args)
    ...
    s_cnt = 0
    score_list = []
    for cnt, (log_vecs, log_mask, news_vecs) in enumerate(dataloader):
        ...
        for user_vec, news_vec in zip(user_vecs, news_vecs):
        ...
        if cnt % args.log_steps == 0:
            ...
            with open("../score.txt", 'a') as f:
                # f.writelines(map(str, score_list))
                for s in score_list :
...
if __name__ == "__main__":
    ...
    if 'train' in args.mode:
        ...
        if args.nGPU == 1:
            logging.info('Start to training ...')
            train(None, args)
        else:
        ...
        if args.prepare:
            logging.info('Preparing valadation data...')
        ...
        if args.nGPU == 1:
            ...
            torch.multiprocessing.spawn(val, nprocs=args.nGPU, args=(args,))
